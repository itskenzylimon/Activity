...
String get separator => pathContext.separator;
...
String absolute(String part1,
        ...
        String? part14,
        String? part15]) =>
    pathContext.absolute(part1, part2, part3, part4, part5, part6, part7, part8,
...
///
///     p.basename('path/to/'); // -> 'to'
String basename(String path) => pathContext.basename(path);
...
String basenameWithoutExtension(String path) =>
    pathContext.basenameWithoutExtension(path);
...
///     p.dirname('foo');  // -> '.'
///     p.dirname('');  // -> '.'
String dirname(String path) => pathContext.dirname(path);
...
String extension(String path, [int level = 1]) =>
    pathContext.extension(path, level);
...
///       // -> 'https://dart.dev'
String rootPrefix(String path) => pathContext.rootPrefix(path);
...
bool isAbsolute(String path) => pathContext.isAbsolute(path);

/// Returns `true` if [path] is a relative path and `false` if it is absolute.
/// On POSIX systems, absolute paths start with a `/` (forward slash). On
/// Windows, an absolute path starts with `\\`, or a drive letter followed by
/// `:/` or `:\`.
bool isRelative(String path) => pathContext.isRelative(path);
...
bool isRootRelative(String path) => pathContext.isRootRelative(path);
...
String join(String part1,
        ...
        String? part15,
        String? part16]) =>
    pathContext.join(part1, part2, part3, part4, part5, part6, part7, part8, part9,
...
String joinAll(Iterable<String> parts) => pathContext.joinAll(parts);
...
List<String> split(String path) => pathContext.split(path);
...
String canonicalize(String path) => pathContext.canonicalize(path);
...
String normalize(String path) => pathContext.normalize(path);
...
String relative(String path, {String? from}) =>
    pathContext.relative(path, from: from);
...
bool isWithin(String parent, String child) => pathContext.isWithin(parent, child);
...
/// semantics.
bool equals(String path1, String path2) => pathContext.equals(path1, path2);
...
int hash(String path) => pathContext.hash(path);

/// Removes a trailing extension from the last part of [path].
///
///     p.withoutExtension('path/to/foo.dart'); // -> 'path/to/foo'
String withoutExtension(String path) => pathContext.withoutExtension(path);
...
String setExtension(String path, String extension) =>
    pathContext.setExtension(path, extension);
...
String fromUri(Object? uri) => pathContext.fromUri(uri!);
...
Uri toUri(String path) => pathContext.toUri(path);
...
String prettyUri(Object? uri) => pathContext.prettyUri(uri!);

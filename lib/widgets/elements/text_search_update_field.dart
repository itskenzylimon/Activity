                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            if (parameters['type'] == 'dropdown') {
                              ...
                              if (widget.valueFormResults.containsKey(
                                      '${item['parameters'][index]['name']}-${callbackElement['outputs'][0]['value']}') ==
                                  false) {
                                // setState(() {
                                /// Add the value
                                widget.valueFormResults.putIfAbsent(
                                    '${item['parameters'][index]['name']}-${callbackElement['outputs'][0]['value']}',
                                    () => newValue);
                                // });
                              }
                            ...
                            return Container(
                                child: parameters['type'] == 'dropdown'
                                    ? DropDownIPRSWidget(
                                        ...
                                        elementName:
                                            '${item['parameters'][index]['name']}-${callbackElement['outputs'][0]['value']}',
                                        valueFormResults: widget.valueFormResults,
                                        customTheme: {},
                                      )
                                    : TextFieldIPRSWidget(
                                        onElementCallback: (Map<String, dynamic> value) {
                                    ...
                                    if (data['first_name'].toLowerCase() ==
                                        ...
                                        if (widget.valueFormResults.containsKey(i['value'])) {
                                          ...
                                          setState(() {
                                            ...
                                            switch (_textSelect(i['text'])) {
                                              case 'dob':
                                                widget.valueFormResults[i['value']]!['value'] =
                                              ...
                                              default:
                                                widget.valueFormResults[i['value']]!['controller']
                                                    .text = data['$resData'] ?? '';
                                                widget.valueFormResults[i['value']]!['value'] =
                                                    data['$resData'] ?? '';
                                            }
                                      ...
                                      displayInfoBar(contextMode, builder: (context, close) {
                                        return InfoBar(
                                          title: const Text('IPRS LOOKUP'),
                                          content:
                                              const Text('First Name does not match particulars'),
                                          action: IconButton(
                                            icon: const Icon(FluentIcons.clear),
                                            onPressed: close,
                                          ),
